...
const addValues = (arr) => {
  // Solution code here...
  let newarr = arr.reduce((accumulator,cuurentValue) => {
...
const returnNames = (arr) => {
  ...
  arr.reduce((accumulator,currentValue) => {
    characters.push(currentValue.name);
  }, 0)
  return characters;
};
...
const reversedString = (str) => {
  ...
  let reversarr = string.reduce((accumulator,currentValue) => {
...
const extractChildren = (arr) => {
  // Solution code here...
  let char = arr.filter(element => {
    if (element.name.includes('a')) {
      return element;
    }
  })
  let childerNames = [];
  char.reduce((accumulator,currentValue) => {
    if (currentValue.hasOwnProperty('children')) {
