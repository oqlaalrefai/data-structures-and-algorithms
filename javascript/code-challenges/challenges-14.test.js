...
const toTitleCase = (arr) => {
  // Solution code here...
  let uppercaselet = arr.map((element) => {
    let delimeter = element.split("");
    delimeter[0] = delimeter[0].toUpperCase();
...
Instead of trying to write crazy for loops to automate checking the rows, columns and diagonals consider writing one helper function that accepts three coordinate pairs and checks the items of the array at those locations. For instance helpCheck(row1, col1, row2, col2, row3, col3).
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i < 9; i + 3) {
    if (arr[i] == arr[i + 2] && arr[i] == arr[i + 1] && arr[i] != '') {
      return true;}
       else {
      for (let i = 0; i < 3; i++) {
        if (arr[i] == arr[i + 6] && arr[i] == arr[i + 3] &&  arr[i] != '') {
          return true;
        } else {
          for (let i = 0; i < 3; i + 2) {
            if (arr[i] == arr[8 - i] &&  arr[i] == arr[4] && arr[i] != '') {
              return true;
            }
            else {
              return false; }}
...
describe('Testing challenge 1', () => {
  test('It should return a arr of names', () => {
