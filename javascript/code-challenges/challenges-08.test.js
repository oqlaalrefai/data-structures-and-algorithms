...
const isNum = (input) => {
  // Solution code here...
  const num = /[0-9]/;
  const num_exist = num.test(input);
  return num_exist;
};
...
const containsWorld = (input) => {
  // Solution code here...
  const word = /world/;
  const word_exist = word.test(input);
  return word_exist;
};
...
const isCapitalized = (str) => {
  ...
  const capital_test = str.match(capital);
  return capital_test || [];
};
