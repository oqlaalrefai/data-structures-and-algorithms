...
const validateWord = (word) => {
  // Solution code here...
  let numChar = /\b[a-zA-Z]{5,10}\b/;
  return numChar.test(word);
};
...
const hasNumber = (string) => {
  // Solution code here...
  let letterDigit = /[a-zA-z]+[0-9]+/;
  return letterDigit.test(string);
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  let format = /^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/;
